...
public class CustomPostProcessPass : ScriptableRenderPass {
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData) {
        ...
        //---Custom effect here---
        var customEffect = stack.GetComponent<ScreenGradient>();
        // Only process if the effect is active
        if (customEffect.IsActive()) {
            if (material == null) {
                material = CoreUtils.CreateEngineMaterial("Shader Graphs/ScreenGradient");
            }
            ...
            material.SetFloat(Shader.PropertyToID("_min"), customEffect.start.value);
            material.SetFloat(Shader.PropertyToID("_max"), customEffect.end.value);
            material.SetColor(Shader.PropertyToID("_Color"), customEffect.overlayColor.value);
